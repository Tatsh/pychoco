...
Fixes mypy type errors and eliminates `Any` usage in the chocolatey-choco project.
...
1. Fix all mypy errors reported by `uv run mypy choco`
...
Place new TypedDict classes in `choco/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`choco/typing.py` when shared.
...
1. Run `uv run mypy choco` and capture all errors.
...
4. Run `uv run mypy choco` again to verify fixes.
