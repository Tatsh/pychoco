...
If any changed files are under `choco/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `choco/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `choco/commands/main.py`,
...
For Python files, strip the `choco/` prefix and replace `/` with `.` (like module imports).

- Python file `choco/media.py` → `media:`.
- Multiple files under `choco/commands/` → `commands:`.
- Single command file `choco/commands/admin.py` → `commands.admin:`.
